# 定价法功能 - 快速执行指南 ⚡

## ✅ 更新状态

- ✅ **数据库迁移文件**：5 个 SQL 文件已准备就绪
- ✅ **前端函数调用**：21 个文件已全部更新为 `_1120` 后缀
- ✅ **类型定义**：已添加 `unitPrice` 和 `fixed_price` 支持
- ✅ **界面组件**：已添加定价法选项和单位显示

---

## 🎯 需要执行的 SQL 文件（5个，按顺序）

### ✅ 方式一：Supabase CLI（最简单）

```bash
cd C:\Users\admin\Desktop\中科物流跟踪系统逻辑\github\shipment-data-view
supabase db push
```

**说明**：自动执行所有未应用的迁移文件，推荐！

---

### ✅ 方式二：手动执行（Supabase SQL Editor）

在 Supabase Dashboard → SQL Editor 中，按顺序执行：

#### 1️⃣ 基础字段
```
supabase/migrations/20251120_add_fixed_price_calculation_method.sql
```
**作用**：添加 `unit_price` 字段，更新约束

---

#### 2️⃣ 核心重算函数
```
supabase/migrations/20251120_update_recalculate_functions_support_fixed_price.sql
```
**作用**：更新 `batch_recalculate_partner_costs` 和 `auto_recalc_on_payable_cost_change`

---

#### 3️⃣ 有效数量触发器
```
supabase/migrations/20251120_add_trigger_recalc_on_effective_quantity_change.sql
```
**作用**：创建有效数量变化时的自动重算触发器

---

#### 4️⃣ 链路重算函数
```
supabase/migrations/20251120_update_all_recalc_functions_to_1120_with_fixed_price.sql
```
**作用**：创建所有 `_1120` 版本的链路重算函数

---

#### 5️⃣ 特殊操作函数
```
supabase/migrations/20251120_update_modify_and_batch_filter_to_1120.sql
```
**作用**：创建 `modify_logistics_record_chain_with_recalc_1120` 和 `batch_recalculate_by_filter_1120`

---

## 🛡️ 保护逻辑说明（重要！）

### ✅ 正确理解

**只有同时满足以下三个条件的运单才会被重算**：

1. ✅ `payment_status = 'Unpaid'`（未付款）
2. ✅ `invoice_status = 'Uninvoiced'`（未开票）
3. ✅ `receipt_status = 'Unreceived'`（未收款）

### 🔍 SQL 逻辑解释

```sql
-- 检查条件
IF payment_status != 'Unpaid'               -- 已付款？
   OR invoice_status != 'Uninvoiced'        -- 或 已开票？
   OR receipt_status = 'Received' THEN      -- 或 已收款？
    -- 只要有一个是"是"，就跳过重算
    CONTINUE;
END IF;

-- 如果执行到这里，说明：
-- payment_status = 'Unpaid' AND 
-- invoice_status = 'Uninvoiced' AND 
-- receipt_status = 'Unreceived'
-- → 允许重算 ✅
```

### 📊 各种状态对照表

| 付款状态 | 开票状态 | 收款状态 | 是否重算 |
|---------|---------|---------|---------|
| 未付款 | 未开票 | 未收款 | ✅ **允许** |
| **已付款** | 未开票 | 未收款 | ❌ 跳过 |
| 未付款 | **已开票** | 未收款 | ❌ 跳过 |
| 未付款 | 未开票 | **已收款** | ❌ 跳过 |
| **已付款** | **已开票** | 未收款 | ❌ 跳过 |
| 未付款 | **已开票** | **已收款** | ❌ 跳过 |
| **已付款** | 未开票 | **已收款** | ❌ 跳过 |
| **已付款** | **已开票** | **已收款** | ❌ 跳过 |

**结论**：只有第一行（全部未X）才会重算！

---

## ✅ 验证（执行完后运行）

```sql
-- 1. 检查字段
SELECT column_name FROM information_schema.columns 
WHERE table_name = 'project_partners' AND column_name = 'unit_price';
-- 应返回 1 行

-- 2. 检查函数（应返回 9 行）
SELECT proname FROM pg_proc 
WHERE proname IN (
    'batch_recalculate_partner_costs',
    'auto_recalc_on_payable_cost_change',
    'auto_recalc_on_effective_quantity_or_cost_change',
    'recalculate_costs_for_chain_1120',
    'recalculate_costs_for_chain_safe_1120',
    'recalculate_costs_for_project_1120',
    'auto_recalc_on_project_partner_change_1120',
    'modify_logistics_record_chain_with_recalc_1120',
    'batch_recalculate_by_filter_1120'
);

-- 3. 检查触发器（应返回 3 行）
SELECT tgname FROM pg_trigger WHERE tgname LIKE '%recalc%';
```

---

## 🎉 完成！

执行完成后，前端即可使用定价法功能：

1. 打开项目管理页面
2. 创建/编辑项目
3. 选择"定价"计算方法
4. 输入单价（单位会根据计费模式自动显示：元/吨、元/车、元/方、元/件）
5. 保存

---

## 📞 遇到问题？

### 常见错误

**"column already exists"**
- 说明该迁移已执行，跳过即可

**"function does not exist"**
- 说明前面的文件未执行，请按顺序执行

**重算没有触发**
- 检查运单状态：必须是未付款 AND 未开票 AND 未收款

---

**创建日期**：2025-11-20  
**最后更新**：2025-11-20


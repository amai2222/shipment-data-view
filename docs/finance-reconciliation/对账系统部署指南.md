# 对账系统部署指南

## 📋 部署概述

本文档说明如何部署运费对账系统到生产环境。

## 🎯 部署前准备

### 1. 检查依赖

确保以下系统已部署：
- ✅ 统一权限系统（`has_function_permission` 函数）
- ✅ `logistics_partner_costs` 表已存在
- ✅ `logistics_records` 表已存在
- ✅ `partners` 表已存在
- ✅ `auth.users` 表已存在

### 2. 备份数据库

**重要**：在执行迁移前，请先备份数据库！

```sql
-- 备份相关表
pg_dump -t logistics_partner_costs > backup_logistics_partner_costs.sql
```

## 📦 SQL 迁移文件执行顺序

### 执行顺序（重要！）

必须按照以下顺序执行 SQL 文件：

**完整执行顺序**：
1. 添加对账字段和基础函数
2. 迁移权限系统
3. 添加自动对账功能（新增）
4. 更新查询函数

#### 第一步：添加对账字段和基础函数

**文件**：`supabase/migrations/20250131_add_reconciliation_to_partner_costs.sql`

**功能**：
- 为 `logistics_partner_costs` 表添加对账相关字段
- 创建索引
- 创建对账函数（使用旧权限检查）

**执行命令**：
```bash
# 使用 Supabase CLI
supabase db execute -f supabase/migrations/20250131_add_reconciliation_to_partner_costs.sql

# 或直接在数据库客户端执行
psql -h your-host -U your-user -d your-database -f supabase/migrations/20250131_add_reconciliation_to_partner_costs.sql
```

**验证**：
```sql
-- 检查字段是否添加成功
SELECT column_name, data_type, column_default
FROM information_schema.columns
WHERE table_name = 'logistics_partner_costs'
  AND column_name LIKE 'reconciliation%';

-- 检查函数是否创建成功
SELECT proname, prosrc
FROM pg_proc
WHERE proname IN ('reconcile_partner_costs_batch', 'reconcile_partner_cost');
```

---

#### 第二步：迁移权限系统

**文件**：`supabase/migrations/20250131_migrate_reconciliation_permissions_to_unified_system.sql`

**功能**：
- 更新对账函数，使用统一权限系统
- 为 `finance` 角色配置对账权限

**执行命令**：
```bash
supabase db execute -f supabase/migrations/20250131_migrate_reconciliation_permissions_to_unified_system.sql
```

**验证**：
```sql
-- 检查函数是否已更新（应该使用 has_function_permission）
SELECT proname, prosrc
FROM pg_proc
WHERE proname = 'reconcile_partner_costs_batch'
  AND prosrc LIKE '%has_function_permission%';

-- 检查权限配置
SELECT role, function_permissions
FROM role_permission_templates
WHERE role = 'finance'
  AND 'finance.reconcile' = ANY(COALESCE(function_permissions, ARRAY[]::TEXT[]));
```

---

#### 第三步：添加自动对账功能（新增）

**文件**：`supabase/migrations/20250131_add_auto_reconciliation_function.sql`

**功能**：
- 创建 `auto_reconcile_by_waybill_1116` 函数（基于运单号精确匹配）
- 创建 `auto_reconcile_smart_1116` 函数（智能多维度匹配）

**执行命令**：
```bash
supabase db execute -f supabase/migrations/20250131_add_auto_reconciliation_function.sql
```

**验证**：
```sql
-- 检查自动对账函数是否创建成功
SELECT 
    p.proname AS function_name,
    pg_get_function_arguments(p.oid) AS arguments
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public'
  AND p.proname IN ('auto_reconcile_by_waybill_1116', 'auto_reconcile_smart_1116')
ORDER BY p.proname;

-- 测试自动对账函数（可选，建议在测试环境先测试）
-- SELECT public.auto_reconcile_by_waybill_1116(p_partner_id := NULL);
```

---

#### 第四步：更新查询函数

**文件**：`supabase/migrations/20250131_update_reconciliation_function_to_return_status.sql`

**功能**：
- 更新 `get_finance_reconciliation_by_partner_1116` 函数
- 添加对账状态筛选参数
- 返回对账状态信息

**执行命令**：
```bash
supabase db execute -f supabase/migrations/20250131_update_reconciliation_function_to_return_status.sql
```

**验证**：
```sql
-- 检查函数参数是否包含对账状态
SELECT 
    p.proname,
    pg_get_function_arguments(p.oid) as arguments
FROM pg_proc p
WHERE p.proname = 'get_finance_reconciliation_by_partner_1116';

-- 测试查询函数
SELECT public.get_finance_reconciliation_by_partner_1116(
    p_project_id := NULL,
    p_start_date := CURRENT_DATE - INTERVAL '30 days',
    p_end_date := CURRENT_DATE,
    p_page_number := 1,
    p_page_size := 10,
    p_reconciliation_status := 'Unreconciled'
);
```

---

## 📊 完整执行清单

### 快速部署脚本

创建 `deploy_reconciliation_system.sql`：

```sql
-- ============================================================================
-- 对账系统完整部署脚本
-- 执行日期：2025-11-16
-- ============================================================================

-- 开始事务
BEGIN;

-- ============================================================================
-- 第一步：添加对账字段和基础函数
-- ============================================================================
\i supabase/migrations/20250131_add_reconciliation_to_partner_costs.sql

-- ============================================================================
-- 第二步：迁移权限系统
-- ============================================================================
\i supabase/migrations/20250131_migrate_reconciliation_permissions_to_unified_system.sql

-- ============================================================================
-- 第三步：更新查询函数
-- ============================================================================
\i supabase/migrations/20250131_update_reconciliation_function_to_return_status.sql

-- 提交事务
COMMIT;

-- 验证部署
SELECT '✅ 对账系统部署完成' AS status;
```

### 使用 Supabase CLI 部署

```bash
# 1. 进入项目目录
cd /path/to/project

# 2. 按顺序执行迁移文件
supabase db execute -f supabase/migrations/20250131_add_reconciliation_to_partner_costs.sql
supabase db execute -f supabase/migrations/20250131_migrate_reconciliation_permissions_to_unified_system.sql
supabase db execute -f supabase/migrations/20250131_update_reconciliation_function_to_return_status.sql

# 3. 验证部署
supabase db execute -c "SELECT '✅ 对账系统部署完成' AS status;"
```

---

## ✅ 部署后验证

### 1. 检查数据库结构

```sql
-- 检查表字段
SELECT 
    column_name,
    data_type,
    column_default,
    is_nullable
FROM information_schema.columns
WHERE table_name = 'logistics_partner_costs'
  AND column_name LIKE 'reconciliation%'
ORDER BY ordinal_position;

-- 检查索引
SELECT 
    indexname,
    indexdef
FROM pg_indexes
WHERE tablename = 'logistics_partner_costs'
  AND indexname LIKE '%reconciliation%';
```

### 2. 检查函数

```sql
-- 检查所有对账相关函数
SELECT 
    proname AS function_name,
    pg_get_function_arguments(oid) AS arguments,
    pg_get_function_result(oid) AS return_type
FROM pg_proc
WHERE proname IN (
    'reconcile_partner_costs_batch',
    'reconcile_partner_cost',
    'get_finance_reconciliation_by_partner_1116'
)
ORDER BY proname;
```

### 3. 检查权限配置

```sql
-- 检查 finance 角色权限
SELECT 
    role,
    function_permissions
FROM role_permission_templates
WHERE role = 'finance'
  AND 'finance.reconcile' = ANY(COALESCE(function_permissions, ARRAY[]::TEXT[]));
```

### 4. 功能测试

```sql
-- 测试批量对账函数（需要 finance.reconcile 权限）
SELECT public.reconcile_partner_costs_batch(
    ARRAY['test-uuid-1', 'test-uuid-2']::UUID[],
    'Reconciled',
    '测试对账'
);

-- 测试查询函数
SELECT public.get_finance_reconciliation_by_partner_1116(
    p_project_id := NULL,
    p_start_date := CURRENT_DATE - INTERVAL '7 days',
    p_end_date := CURRENT_DATE,
    p_page_number := 1,
    p_page_size := 10,
    p_reconciliation_status := NULL
);
```

---

## 🔧 故障排除

### 问题1：函数名不唯一错误

**错误信息**：
```
ERROR: 42725: function name "public.get_finance_reconciliation_by_partner_1116" is not unique
HINT: Specify the argument list to select the function unambiguously.
```

**原因**：数据库中存在多个同名但参数不同的函数版本。

**解决方案**：

1. **执行清理脚本**：
```bash
psql -h your-host -U your-user -d your-database -f scripts/fix_reconciliation_function_conflict.sql
```

2. **或手动删除旧版本**：
```sql
-- 删除没有 p_reconciliation_status 参数的旧版本
DROP FUNCTION IF EXISTS public.get_finance_reconciliation_by_partner_1116(
    text, date, date, uuid, integer, integer, text, text, text, text, text
);
```

3. **重新执行迁移文件**：
```bash
supabase db execute -f supabase/migrations/20250131_update_reconciliation_function_to_return_status.sql
```

---

## 🔄 回滚方案

如果部署失败，可以按以下步骤回滚：

### 回滚步骤

```sql
-- 1. 删除函数
DROP FUNCTION IF EXISTS public.reconcile_partner_costs_batch(UUID[], TEXT, TEXT);
DROP FUNCTION IF EXISTS public.reconcile_partner_cost(UUID, UUID, TEXT, TEXT);

-- 2. 恢复 get_finance_reconciliation_by_partner_1116 函数（需要从备份恢复）

-- 3. 删除字段（谨慎操作，会丢失数据）
ALTER TABLE public.logistics_partner_costs
DROP COLUMN IF EXISTS reconciliation_status,
DROP COLUMN IF EXISTS reconciliation_date,
DROP COLUMN IF EXISTS reconciliation_by,
DROP COLUMN IF EXISTS reconciliation_notes;

-- 4. 删除索引
DROP INDEX IF EXISTS idx_logistics_partner_costs_reconciliation_status;
DROP INDEX IF EXISTS idx_logistics_partner_costs_reconciliation_date;
```

---

## ⚠️ 注意事项

### 1. 执行顺序

**必须严格按照顺序执行**：
1. 先添加字段和基础函数
2. 再迁移权限系统
3. 最后更新查询函数

### 2. 权限要求

- 执行迁移需要数据库管理员权限
- 确保 `has_function_permission` 函数已存在
- 确保 `role_permission_templates` 表已存在

### 3. 数据安全

- **备份数据库**：执行前必须备份
- **测试环境**：建议先在测试环境执行
- **事务处理**：建议在事务中执行，便于回滚

### 4. 性能影响

- 添加字段时，如果表数据量大，可能需要较长时间
- 创建索引时，可能会锁定表
- 建议在业务低峰期执行

---

## 📝 部署检查清单

### 部署前

- [ ] 已备份数据库
- [ ] 已在测试环境验证
- [ ] 已确认执行顺序
- [ ] 已准备回滚方案

### 部署中

- [ ] 执行第一步：添加字段和基础函数
- [ ] 验证第一步执行成功
- [ ] 执行第二步：迁移权限系统
- [ ] 验证第二步执行成功
- [ ] 执行第三步：更新查询函数
- [ ] 验证第三步执行成功

### 部署后

- [ ] 验证数据库结构
- [ ] 验证函数创建
- [ ] 验证权限配置
- [ ] 功能测试通过
- [ ] 前端功能正常

---

## 🚀 快速部署命令

### 一键部署（推荐）

```bash
# 创建部署脚本
cat > deploy_reconciliation.sh << 'EOF'
#!/bin/bash
echo "开始部署对账系统..."

echo "步骤 1/3: 添加对账字段和基础函数"
supabase db execute -f supabase/migrations/20250131_add_reconciliation_to_partner_costs.sql

echo "步骤 2/3: 迁移权限系统"
supabase db execute -f supabase/migrations/20250131_migrate_reconciliation_permissions_to_unified_system.sql

echo "步骤 3/3: 更新查询函数"
supabase db execute -f supabase/migrations/20250131_update_reconciliation_function_to_return_status.sql

echo "✅ 对账系统部署完成！"
EOF

chmod +x deploy_reconciliation.sh
./deploy_reconciliation.sh
```

---

**文档版本**：v1.0  
**创建日期**：2025-11-16  
**最后更新**：2025-11-16


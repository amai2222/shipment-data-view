...
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT, TEXT, DATE, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered();
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT, TEXT, DATE);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT, TEXT, DATE, INTEGER);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT, TEXT, DATE, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT, TEXT, DATE, TEXT, INTEGER);
DROP FUNCTION IF EXISTS public.get_payment_requests_filtered(TEXT, TEXT, TEXT, DATE, TEXT, INTEGER, INTEGER);

-- 删除其他相关函数
DROP FUNCTION IF EXISTS public.rollback_payment_request_approval(TEXT);
DROP FUNCTION IF EXISTS public.batch_approve_payment_requests(TEXT[]);
DROP FUNCTION IF EXISTS public.batch_pay_payment_requests(TEXT[]);
    ...
    IF p_waybill_number IS NOT NULL AND p_waybill_number != '' OR
       ...
       p_loading_date IS NOT NULL THEN
        ...
        IF v_logistics_ids IS NOT NULL AND array_length(v_logistics_ids, 1) > 0 THEN
            -- 使用 ANY 操作符检查 logistics_record_ids 数组中是否包含任何匹配的ID
            v_where_conditions := array_append(v_where_conditions, 
...
-- 执行完成提示
SELECT '函数名冲突修复完成！申请单筛选函数创建成功！' as message;

...
export function ContractPermissionManager({ 
  ...
  const contractCategories = [
    { value: '行政合同', label: '行政合同', icon: Building2, color: 'bg-blue-100 text-blue-800' },
    { value: '内部合同', label: '内部合同', icon: Users, color: 'bg-green-100 text-green-800' },
  ...
  // 更新分类权限模板
  const handleUpdateCategoryTemplate = async (template: CategoryPermissionTemplate) => {
    try {
      const { error } = await supabase
        .from('contract_category_permission_templates')
        .update({
  ...
  const getCategoryInfo = (category: string) => {
    return contractCategories.find(c => c.value === category) || contractCategories[0];
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">所有者权限</CardTitle>
            <Crown className="h-4 w-4 text-yellow-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-yellow-600">{permissionStats.ownerCount}</div>
            <p className="text-xs text-muted-foreground">合同所有者权限</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="permissions" className="w-full">
        ...
        <TabsContent value="owners" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>合同所有者权限</CardTitle>
              <CardDescription>
                显示所有合同的所有者权限，所有者自动拥有该合同的所有权限
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    {ownerPermissions.map((owner) => {
                      ...
                      return (
                        <TableRow key={owner.id}>
                          <TableCell>
                            <div>
                              <div className="font-medium">{owner.contract_number}</div>
                              <div className="text-sm text-muted-foreground">
                                {owner.counterparty_company}
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <div className="text-sm">
                              {format(new Date(owner.granted_at), 'yyyy-MM-dd HH:mm')}
                            </div>
                          </TableCell>
                          <TableCell>
                            {owner.is_active ? (

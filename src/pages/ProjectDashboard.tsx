...
// 描述: [FINAL-LOGIC-FIX] 最终逻辑修正版。匹配新的后端函数，直接使用单个项目详情，不再进行find操作。
...
interface DriverReportRow { driver_name: string; license_plate: string; phone: string; trip_count: number; total_tonnage: number; total_driver_receivable: number; total_partner_payable: number; total_trips_in_project: number; total_receivable_in_project: number; }
...
// ★★★ 核心修改: 更新 DashboardData 类型以匹配新的SQL函数返回结构 ★★★
interface DashboardData { 
  project_details: {
    selected: ProjectDetails | null;
    all_for_dropdown: { id: string; name: string }[] | null;
  } | null;
...
export default function ProjectDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase.rpc('get_project_dashboard_data_v2' as any, { p_selected_project_id: projectId, p_report_date: format(reportDate, 'yyyy-MM-dd') });
  ...
  // ★★★ 核心修改: 不再需要 find 操作，直接获取 ★★★
  const selectedProjectDetails = dashboardData?.project_details?.selected;
  const allProjectsForDropdown = dashboardData?.project_details?.all_for_dropdown || [];

  const unitConfig = useMemo(() => {
    if (!selectedProjectDetails) return {}; // 返回空对象以避免后续错误
    ...
    switch (billingType) {
      case 2: return { progressUnit: '车', progressCompleted: stats?.total_trips || 0, progressPlanned: selectedProjectDetails.planned_total_tons || 1, dailyReportValue: daily?.trip_count || 0, trendLineLabel: '总车次', driverReportColHeader: '卸货车次', getDriverReportRowValue: (row: DriverReportRow) => row.trip_count, };
      case 3: return { progressUnit: '立方', progressCompleted: stats?.total_tonnage || 0, progressPlanned: selectedProjectDetails.planned_total_tons || 1, dailyReportValue: daily?.total_tonnage || 0, trendLineLabel: '总立方', driverReportColHeader: '卸货立方', getDriverReportRowValue: (row: DriverReportRow) => row.total_tonnage, };
      default: return { progressUnit: '吨', progressCompleted: stats?.total_tonnage || 0, progressPlanned: selectedProjectDetails.planned_total_tons || 1, dailyReportValue: daily?.total_tonnage || 0, trendLineLabel: '总重量', driverReportColHeader: '卸货吨数', getDriverReportRowValue: (row: DriverReportRow) => row.total_tonnage, };
    }
  }, [selectedProjectDetails, dashboardData]);

  const progressPlannedSafe = unitConfig.progressPlanned || 1;
  const progressPercentage = ((unitConfig.progressCompleted || 0) / progressPlannedSafe) * 100;
  ...
  if (!selectedProjectDetails) {
    return (
      <div className="p-6 bg-slate-50 min-h-screen">
        ...
        <div className="text-center py-10 text-slate-500">当前项目数据不存在或加载失败。请确认URL中的项目ID是否正确，或从下拉框选择一个项目。</div>
        <Select onValueChange={(newId) => navigate(`/project/${newId}`)}>
          <SelectTrigger className="w-[250px] mx-auto mt-4"><SelectValue placeholder="选择其他项目..." /></SelectTrigger>
          <SelectContent>{allProjectsForDropdown.map(p => (<SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>))}</SelectContent>
        </Select>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 bg-slate-50 space-y-6">
      ...
      {/* ... (剩余的JSX代码与 [CRASH-PROOF-FINAL] 版本完全相同，这里不再重复) ... */}

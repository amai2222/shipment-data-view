...
export default function DataImportWithDuplicateCheck() {
  ...
  const handleTemplateDownload = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      // 表头 - 标明必填和可选字段
      ['项目名称*', '合作链路(可选)', '司机姓名*', '车牌号(可选)', '司机电话(可选)', '装货地点*', '卸货地点*', '装货日期*', '卸货日期(可选)', '装货数量(可选)', '卸货数量(可选)', '运费金额(可选)', '额外费用(可选)', '运输类型(可选)', '备注(可选)', '其他平台名称(可选)', '其他平台运单号(可选)', '其他平台运单号说明(可选)'],
      // 字段说明行
      ['必填', '可选', '必填', '可选', '可选', '必填', '必填', '必填', '可选', '可选', '可选', '可选', '可选', '可选(默认:实际运输)', '可选', '可选', '可选', '可选'],
      // 计费类型说明行
      ['', '', '', '', '', '', '', '', '', '根据合作链路计费类型动态显示: 重量(吨)/发车次数/体积(立方)', '根据合作链路计费类型动态显示: 重量(吨)/发车次数/体积(立方)', '', '', '', '', '', '', ''],
      // 示例数据
      ['示例项目A', '默认链路', '张三', '京A12345', '13800138000', '北京仓库', '上海仓库', '2024-01-15', '2024-01-16', '10.5', '10.2', '5000', '200', '实际运输', '正常运输', '平台A,平台B', '运单1|运单2,运单3', '逗号分隔不同平台，竖线分隔同一平台的多个运单号'],
      ['示例项目B', '', '李四', '沪B67890', '13900139000', '上海仓库', '广州仓库', '2024-01-17', '', '15.0', '14.8', '8000', '300', '实际运输', '加急运输', '平台C', '运单4|运单5|运单6', '同一平台的多个运单号用竖线分隔'],
      ['示例项目C', '特殊链路', '王五', '粤C11111', '13700137000', '广州仓库', '深圳仓库', '2024-01-18', '2024-01-19', '8.0', '7.9', '4000', '100', '实际运输', '标准运输', '', '', '可以不填写平台信息'],
      ['示例项目D', '默认链路', '赵六', '京D22222', '13600136000', '北京仓库', '天津仓库', '2024-01-20', '2024-01-21', '12.0', '11.8', '6000', '250', '实际运输', '混合运输', '平台A,平台B,平台C', '运单7,运单8|运单9,运单10|运单11', '复杂示例：平台A有运单7，平台B有运单8和运单9，平台C有运单10和运单11']
    ]);

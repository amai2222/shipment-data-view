...
// 描述: [g81gf 最终交付版] 终极前端代码，直接调用大一统RPC，自包含所有逻辑，渲染所有UI。
...
import { supabase } from "@/integrations/supabase/client"; // 直接导入supabase客户端
...
const initialSummaryStats: SummaryStats = {
  yesterday_trip_count: 0,
  yesterday_loading_weight: 0,
  yesterday_payable_amount: 0,
  yesterday_invoice_amount: 0,
  unsettled_over_30_days_amount: 0,
  unsettled_over_60_days_amount: 0,
  unsettled_over_90_days_amount: 0,
  uninvoiced_over_30_days_amount: 0,
  uninvoiced_over_60_days_amount: 0,
  uninvoiced_over_90_days_amount: 0,
};
  ...
  const fetchDialogRecords = useCallback(async () => {
    ...
    try {
      ...
      let query = supabase
        ...
        .lt('created_at', new Date(new Date(endDate).getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0]);
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 border rounded-lg">
              <p className="text-sm text-muted-foreground">发车车次</p>
              <p className="text-2xl font-bold">{summaryStats.yesterday_trip_count}</p>
            </div>
            <div className="p-4 border rounded-lg">
              <p className="text-sm text-muted-foreground">装货吨数</p>
              <p className="text-2xl font-bold">{formatCurrency(summaryStats.yesterday_loading_weight, 2)}</p>
            </div>
            <div className="p-4 border rounded-lg">
              <p className="text-sm text-muted-foreground">应收金额</p>
              <p className="text-2xl font-bold">{formatCurrency(summaryStats.yesterday_payable_amount, 2)}</p>
            </div>
            <div className="p-4 border rounded-lg">
              <p className="text-sm text-muted-foreground">开票金额</p>
              <p className="text-2xl font-bold">{formatCurrency(summaryStats.yesterday_invoice_amount, 2)}</p>
            </div>
          </div>
        </CardContent>
      </Card>

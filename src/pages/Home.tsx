...
// 描述: [最终模式重构版] 1. 实现“用户驱动”模式，页面瞬间加载，默认不请求任何数据。 2. 添加“加载/刷新”按钮，由用户手动触发数据加载。 3. 修复默认开始日期为当年1月1日。
...
export default function Home() {
  ...
  // [核心修改] 默认不加载，所以 isLoading 初始为 false
  const [isLoading, setIsLoading] = useState(false);
  const [isDataLoaded, setIsDataLoaded] = useState(false); // 新增状态，判断是否已加载过数据
  ...
  // [核心修改] useEffect 不再自动加载数据，只检查迁移状态
  useEffect(() => {
    checkMigrationStatus();
  }, []);

  // [核心修改] loadData 现在只由用户手动触发
  const loadData = async () => {
    ...
    try {
      ...
      setIsDataLoaded(true); // 标记数据已成功加载

      toast({
        title: "数据加载成功",
        description: "已从数据库获取最新数据。",
        action: <CheckCircle className="h-5 w-5 text-green-500" />,
      });

    } catch (error) {
      console.error('Supabase数据加载失败:', error);
      toast({
        title: "数据加载失败",
        description: "无法连接到数据库，请检查网络或联系管理员。",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const recordsByProject = useMemo(() => {
    return filteredRecords.reduce((acc, record) => {
      if (!acc[record.projectId]) { acc[record.projectId] = []; }
  ...
  const projectStats = useMemo(() => {
    if (selectedProjectId && selectedProjectId !== 'all') {
      ...
      return [{ projectId: selectedProjectId, project, projectRecords, dailyTransportStats: generateDailyTransportStats(projectRecords), dailyCostStats: generateDailyCostStats(projectRecords), dailyCountStats: generateDailyCountStats(projectRecords), legendTotals: generateLegendTotals(projectRecords), isAllProjects: false }];
    } else {
      return [{ projectId: 'all', project: { name: '所有项目', manager: '全部负责人' } as Project, projectRecords: filteredRecords, dailyTransportStats: generateDailyTransportStats(filteredRecords), dailyCostStats: generateDailyCostStats(filteredRecords), dailyCountStats: generateDailyCountStats(filteredRecords), legendTotals: generateLegendTotals(filteredRecords), isAllProjects: true }];
    }
  }, [recordsByProject, projects, selectedProjectId, filteredRecords]);

  const selectedRecords = useMemo(() => {
    if (!selectedProjectId) return [];
    let projectRecords: LogisticsRecord[] = (selectedProjectId === 'all') ? filteredRecords : recordsByProject[selectedProjectId] || [];
  ...
  const handleLegendClick = (projectId: string) => {
    ...
    if (projectId === 'all' || !selectedProjectId) { setSelectedProjectId('all'); } else { setSelectedProjectId(projectId); }
    setIsDetailDialogOpen(true);
  };

  const handleDialogClose = (open: boolean) => {
    setIsDetailDialogOpen(open);
    if (!open) { setSelectedDate(null); setSelectedProjectId(null); }
  };
  ...
  return (
    <div className="space-y-8 p-4 md:p-8">
      ...
      <div className="flex justify-between items-center bg-gradient-primary p-6 rounded-lg shadow-primary text-primary-foreground">
        <div>
        ...
        {/* [核心修改] 添加手动加载/刷新按钮 */}
        <Button onClick={loadData} disabled={isLoading} size="lg">
          <RefreshCw className={`mr-2 h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />
          {isLoading ? '正在加载...' : (isDataLoaded ? '刷新数据' : '加载数据')}
        </Button>
      </div>
      ...
      {isDataLoaded ? (
        ...
        <div className="text-center py-20 bg-slate-50 rounded-lg">
          ...
          <p className="text-slate-500 mt-2">请点击右上角的“加载数据”按钮以开始分析。</p>
        </div>
      )}
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={handleDialogClose}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto" aria-describedby="dialog-description">
          <DialogHeader>
            <DialogTitle className="flex items-center"><Eye className="mr-2 h-5 w-5" />{selectedDate ? `${new Date(selectedDate).toLocaleDateString('zh-CN')} 详细运输记录` : selectedProjectId === 'all' ? '所有项目运输记录' : `${projects.find(p => p.id === selectedProjectId)?.name || '项目'} 全部运输记录`}</DialogTitle>
            <div id="dialog-description" className="sr-only">显示运输记录详细信息</div>
          </DialogHeader>
          {selectedRecords.length > 0 ? (
            <div className="space-y-4">
              <div className="bg-blue-50 p-4 rounded-lg"><h3 className="font-semibold text-blue-900">数据概览</h3><div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2"><div><p className="text-sm text-blue-700">运输次数</p><p className="text-lg font-bold text-blue-900">{selectedRecords.length}</p></div><div><p className="text-sm text-blue-700">总重量</p><p className="text-lg font-bold text-blue-900">{selectedRecords.reduce((sum, r) => sum + r.loadingWeight, 0).toFixed(1)}吨</p></div><div><p className="text-sm text-blue-700">总费用</p><p className="text-lg font-bold text-blue-900">¥{selectedRecords.reduce((sum, r) => sum + (r.currentFee || 0) + (r.extraFee || 0), 0).toFixed(2)}</p></div><div><p className="text-sm text-blue-700">实际运输/退货</p><p className="text-lg font-bold text-blue-900">{selectedRecords.filter(r => r.transportType === "实际运输").length}/{selectedRecords.filter(r => r.transportType === "退货").length}</p></div></div></div>
              <div className="border rounded-lg overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {selectedRecords.sort((a, b) => { const projectNameA = a.projectName || ''; const projectNameB = b.projectName || ''; const projectCompare = projectNameB.localeCompare(projectNameA, 'zh-CN'); if (projectCompare !== 0) return projectCompare; return b.autoNumber.localeCompare(a.autoNumber, 'zh-CN'); }).map((record) => (
          ...
          ) : (<div className="text-center py-8 text-muted-foreground">该日期没有运输记录</div>)}
        </DialogContent>
      </Dialog>
    </div>
  );

            ...
            <>
              <div className="space-y-2">
                ...
                <div className="flex items-center border-b pb-2 text-sm font-medium text-muted-foreground">
                  ...
                  <div className="flex-1 min-w-[200px]">项目名称 / 车次</div>
                ...
                {records.map((record) => (
                  <div key={record.id} className="flex items-center border rounded-lg p-2 hover:bg-muted/50 transition-colors">
                    <div className="w-12 flex-shrink-0 flex items-center justify-center">
                      <Checkbox checked={selectedRecordIds.has(record.id)} onCheckedChange={() => handleSelectRecord(record.id)} />
                    </div>
                    <div className="flex-1 min-w-[200px]">
                      <p className="font-semibold">{record.project_name}</p>
                      <p className="text-sm text-muted-foreground">第 {record.trip_number} 车次</p>
                    </div>
                    <div className="w-32 flex-shrink-0 text-sm">{format(new Date(record.loading_date), 'yyyy-MM-dd')}</div>
                    ...
                    <div className="w-28 flex-shrink-0 text-sm">{record.valid_quantity ? `${record.valid_quantity} 吨` : 'N/A'}</div>
                    <div className="w-28 flex-shrink-0 text-center">
                      {record.image_urls.length > 0 ? (
                        <Button variant="ghost" size="sm" onClick={() => handleImageClick(record.image_urls)}>
                          <ImageIcon className="h-4 w-4 mr-2" />{record.image_urls.length}
                        </Button>
                    ...
                    <div className="w-40 flex-shrink-0 flex justify-end items-center gap-2">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteTarget(record)}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                ))}
              </div>

...
export default function ScaleRecords() {
  ...
  useEffect(() => { loadProjects(); loadDrivers(); loadRecords(); }, [activeFilters]);
  ...
  // ★ 核心修正: 已恢复到您原始的、安全可靠的函数逻辑
  const handleRecordAdded = () => {
    setShowAddDialog(false);
    loadRecords(); // 重新加载整个列表，确保新添加的数据格式完整
  ...
  return (
    <div className="space-y-6">
          ...
          {loading ? ( <div className="text-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto"></div><p className="mt-2 text-muted-foreground">加载中...</p></div> ) : 
           records.length === 0 ? ( <div className="text-center py-8"><p className="text-muted-foreground">暂无磅单记录</p></div> ) : (
            <div className="space-y-4">
              {records.map((record) => (
                <div key={record.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => record.image_urls.length > 0 && handleImageClick(record.image_urls)}>
                  <div className="flex justify-between items-start">
                    <div className="flex-grow space-y-2">
                      <div className="flex items-center gap-2 flex-wrap"><h3 className="font-semibold text-lg">{record.project_name}</h3><Badge variant="outline">第{record.trip_number}车次</Badge>{record.image_urls.length > 0 && (<Badge variant="secondary" className="flex items-center gap-1"><ImageIcon className="h-3 w-3" />{record.image_urls.length}张图片</Badge>)}</div>
                      <div className="text-sm text-muted-foreground flex items-center gap-4 flex-wrap"><p>装车日期: {format(new Date(record.loading_date), 'yyyy-MM-dd')}</p>{record.license_plate && <p>车牌号: {record.license_plate}</p>}{record.driver_name && <p>司机: {record.driver_name}</p>}{record.valid_quantity && <p>有效数量: {record.valid_quantity}吨</p>}</div>
                    </div>
                    <div className="flex flex-col items-end space-y-2 ml-4"><p className="text-sm text-muted-foreground whitespace-nowrap">{format(new Date(record.created_at), 'yyyy-MM-dd HH:mm')}</p><Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={(e) => { e.stopPropagation(); setDeleteTarget(record); }}><Trash2 className="h-4 w-4" /></Button></div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <AlertDialog open={deleteTarget !== null} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>确定要删除吗？</AlertDialogTitle><AlertDialogDescription>此操作将永久删除该条磅单记录及其所有关联图片。此操作无法撤销。</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel onClick={() => setDeleteTarget(null)}>取消</AlertDialogCancel><AlertDialogAction onClick={() => handleDelete(deleteTarget!)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">删除</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  );

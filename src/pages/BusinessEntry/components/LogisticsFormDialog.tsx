    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl">
        ...
        <div className="grid grid-cols-4 gap-4 py-4">
          ...
          <div className="space-y-1"><Label>司机 *</Label><CreatableCombobox options={(filteredDrivers || []).map(d => ({ value: d.id, label: `${d.name} (${d.license_plate || '无车牌'})` }))} value={formData.driver_id} onValueChange={(value) => { const driver = filteredDrivers.find(d => d.id === value); if (driver) { dispatch({ type: 'SET_DRIVER', payload: driver }); } else { handleInputChange('driver_id', value); handleInputChange('driver_name', value); } }} placeholder="选择或创建司机" searchPlaceholder="搜索或输入新司机..." onCreateNew={() => navigate('/drivers')} /></div>

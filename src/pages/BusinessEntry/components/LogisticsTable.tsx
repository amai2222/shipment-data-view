...
export function LogisticsTable({ records, loading, summary, pagination, setPagination, onEdit, onDelete, onView }: LogisticsTableProps) {
              ...
              records.map((record) => (
                <TableRow key={record.id} onClick={() => onView(record)} className="cursor-pointer hover:bg-muted/50">
                  ...
                  <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                    <Button variant="ghost" size="icon" onClick={() => onEdit(record)}><Edit className="h-4 w-4" /></Button>

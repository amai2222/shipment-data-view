...
export function useInvoiceData() {
  ...
  const fetchReportData = useCallback(async (filters: InvoiceFilters) => {
    setLoading(true);
    try {
      const statusArray = filters.invoiceStatus === 'all' ? null : [filters.invoiceStatus];
      ...
      const { data, error } = await supabase.rpc('get_invoice_request_data', {
        ...
        p_driver_receivable: filters.driverReceivable === 'all' ? null : filters.driverReceivable,

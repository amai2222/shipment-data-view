...
const getInvoiceStatusBadge = (status?: string | null) => {
  switch (status) {
    case 'Uninvoiced': return <Badge variant="outline" className="bg-gray-50">未开票</Badge>;
    case 'Processing': return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">开票中</Badge>;
    case 'Invoiced': return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">已开票</Badge>;
        ...
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="min-w-[120px] hidden md:table-cell cursor-pointer hover:bg-muted/50" onClick={() => onSort('loading_date')}>
              <div className="flex items-center gap-1">
                装货日期 {sortField === 'loading_date' && (sortDirection === 'asc' ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />)}
              </div>
            </TableHead>
            ...
            <TableHead className="min-w-[80px]">开票状态</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {records.map((r) => (
            <TableRow key={r.id} data-state={selection.selectedIds.has(r.id) && "selected"}>
              ...
              <TableCell className="font-mono cursor-pointer font-bold text-primary" onClick={() => onRecordClick(r)}>
                {formatCurrency(
                  r.payable_cost ||
                  (r.partner_costs?.find(c => c.level === 0)?.payable_amount) ||
                  r.current_cost ||
                  null
                )}
              </TableCell>
          ...
          <InvoiceSummaryRow
            totalInvoiceableCost={overview?.total_invoiceable_cost || 0}
            partners={partners}

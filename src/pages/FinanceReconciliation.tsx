// 最终文件路径: src/pages/FinanceReconciliation.tsx
...
export default function FinanceReconciliation() {
  ...
  const dateRangeValue: DateRange | undefined = (uiFilters.startDate || uiFilters.endDate)
    ? { from: uiFilters.startDate ? new Date(uiFilters.startDate) : undefined, to: uiFilters.endDate ? new Date(uiFilters.endDate) : undefined }
    : undefined;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <ConfirmDialog title="确认批量重算" description={`您确定要为选中的 ${selectedRecordIds.size} 条运单重新计算所有合作方的应付金额吗？此操作会根据最新的项目合作链路配置覆盖现有数据。`} onConfirm={handleBatchRecalculate}>
          <Button variant="destructive" disabled={selectedRecordIds.size === 0 || isRecalculating}>{isRecalculating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}一键重算已选运单 ({selectedRecordIds.size})</Button>
        </ConfirmDialog>
      </div>

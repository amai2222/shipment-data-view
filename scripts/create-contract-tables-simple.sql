-- 创建合同相关表（简化版本）
-- 这个版本不依赖可能不存在的表，使用更简单的结构

-- 1. 创建合同权限表（简化版本）
CREATE TABLE IF NOT EXISTS public.contract_permissions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    contract_id UUID NOT NULL REFERENCES public.contracts(id) ON DELETE CASCADE,
    user_id UUID, -- 不设置外键约束，避免依赖问题
    role_name TEXT, -- 使用角色名称而不是角色ID
    department_name TEXT, -- 使用部门名称而不是部门ID
...
CREATE INDEX IF NOT EXISTS idx_contract_permissions_role_name ON public.contract_permissions(role_name);
CREATE INDEX IF NOT EXISTS idx_contract_permissions_department_name ON public.contract_permissions(department_name);

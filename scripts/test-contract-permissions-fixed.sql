-- 修复后的合同权限管理系统测试脚本
-- 文件: scripts/test-contract-permissions-fixed.sql
-- 修复了is_active字段不存在的问题
...
-- 7. 测试权限统计功能（修复后）
SELECT '=== 测试权限统计功能 ===' as test_section;

-- 测试权限统计
SELECT 
    '权限统计功能测试' as test_name,
    total_permissions,
    active_permissions,
    expired_permissions,
    user_permissions,
...
BEGIN
    ...
    RAISE NOTICE 'contract_permissions表is_active字段: %', CASE WHEN has_is_active_field THEN '存在' ELSE '不存在（使用兼容模式）' END;
    ...
    IF total_tables >= 3 AND total_functions >= 6 AND total_triggers >= 1 AND total_policies >= 3 AND total_templates >= 3 THEN
        ...
        IF NOT has_is_active_field THEN
            RAISE NOTICE 'ℹ️ 注意：contract_permissions表缺少is_active字段，系统使用兼容模式运行';
        END IF;
    ELSE

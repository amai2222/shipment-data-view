-- 修复时区一致性问题
-- 确保前端显示、数据库存储、验重逻辑都使用中国时区（UTC+8）

-- 1. 更新 preview_import_with_duplicates_check 函数，使用中国时区
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            -- 例如：'2025-09-06' -> '2025-09-06 00:00:00+08:00'
...
-- 2. 更新 batch_import_logistics_records 函数，使用中国时区
...
BEGIN
    -- 将UTC时间转换为中国时区（UTC+8）
    RETURN input_timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Shanghai';
END;
$$;
...
AS $$
BEGIN
    -- 将timestamptz转换为中国时区并格式化
    RETURN to_char(input_timestamp AT TIME ZONE 'Asia/Shanghai', format_str);
END;
$$;

-- 6. 添加辅助函数注释
COMMENT ON FUNCTION public.to_china_timezone(timestamptz) IS '将UTC时间转换为中国时区（UTC+8）';
COMMENT ON FUNCTION public.format_china_date(timestamptz, text) IS '将timestamptz格式化为中国时区的日期字符串';

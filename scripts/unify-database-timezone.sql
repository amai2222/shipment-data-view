-- 统一数据库时区为中国时区（UTC+8）
-- 将现有的timestamptz数据转换为中国时区

-- 1. 备份现有数据（可选，建议在生产环境执行）
-- CREATE TABLE logistics_records_backup AS SELECT * FROM logistics_records;

-- 2. 检查当前数据状态
SELECT '=== 转换前数据状态 ===' as status_title;
SELECT 
    COUNT(*) as total_records,
    COUNT(loading_date) as records_with_loading_date,
    COUNT(unloading_date) as records_with_unloading_date,
    MIN(loading_date) as min_loading_date,
    MAX(loading_date) as max_loading_date
FROM logistics_records;

-- 3. 显示转换前的时区情况
SELECT '=== 转换前时区情况 ===' as status_title;
SELECT 
    auto_number,
    loading_date,
...
-- 5. 检查转换后数据状态
SELECT '=== 转换后数据状态 ===' as status_title;
SELECT 
    COUNT(*) as total_records,
    COUNT(loading_date) as records_with_loading_date,
    COUNT(unloading_date) as records_with_unloading_date,
...
-- 6. 显示转换后的时区情况
SELECT '=== 转换后时区情况 ===' as status_title;
SELECT 
    auto_number,
    loading_date,
    EXTRACT(timezone_hour FROM loading_date) as timezone_offset,
...
-- 7. 验证转换结果
SELECT '=== 转换结果验证 ===' as status_title;
SELECT 
    '所有loading_date都是中国时区' as check_item,
    CASE 
        WHEN COUNT(*) = COUNT(CASE WHEN EXTRACT(timezone_hour FROM loading_date) = 8 THEN 1 END)
        THEN '✓ 通过'
        ELSE '✗ 失败'
    END as result
FROM logistics_records 
...
UNION ALL
SELECT 
    '所有unloading_date都是中国时区' as check_item,
    CASE 
        WHEN COUNT(*) = COUNT(CASE WHEN EXTRACT(timezone_hour FROM unloading_date) = 8 THEN 1 END)
        THEN '✓ 通过'
        ELSE '✗ 失败'
    END as result
FROM logistics_records 
...
BEGIN
    ...
    parsed_data AS (
        SELECT
            ...
            COALESCE(NULLIF(rec->>'unloading_date', ''), rec->>'loading_date' || ' 00:00:00+08:00')::timestamptz AS unloading_date,
    ...
    final_records AS (
        SELECT 
            ...
            pd.loading_location,
            pd.unloading_location,
            pd.loading_date,        -- 已经是正确的中国时区timestamptz类型
            pd.unloading_date,      -- 已经是正确的中国时区timestamptz类型
            pd.loading_weight,
            pd.unloading_weight,
            pd.current_cost,
            pd.extra_cost,
...
-- 11. 最终验证
SELECT '=== 最终验证结果 ===' as status_title;
SELECT 
    '数据库时区统一完成' as result,
    COUNT(*) as total_records,
    COUNT(CASE WHEN EXTRACT(timezone_hour FROM loading_date) = 8 THEN 1 END) as china_timezone_records,
    ROUND(COUNT(CASE WHEN EXTRACT(timezone_hour FROM loading_date) = 8 THEN 1 END) * 100.0 / COUNT(*), 2) as percentage
FROM logistics_records 
WHERE loading_date IS NOT NULL;

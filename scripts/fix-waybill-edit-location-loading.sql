-- 修复运单编辑时卸货地点不能读取的问题
-- 问题：在编辑运单时，地点数据可能还没有加载完成就尝试填充表单

-- 1. 检查logistics_records表中的地点数据格式
SELECT 
    auto_number,
    loading_location,
    unloading_location,
...
BEGIN
    ...
    LOOP
        -- 检查地点是否存在
        IF NOT EXISTS (SELECT 1 FROM locations WHERE name = TRIM(rec.location_name)) THEN
            -- 创建缺失的地点
            INSERT INTO locations (name, created_at)
            VALUES (TRIM(rec.location_name), NOW())
            ON CONFLICT (name) DO NOTHING;
            
            RAISE NOTICE '创建地点: %', TRIM(rec.location_name);
        END IF;
    END LOOP;
END;
$$;

-- 修复导入函数，移除从 projects 表获取 billing_type_id 的错误逻辑
-- projects 表没有 billing_type_id 字段，应该只从 partner_chains 表获取
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            -- 第1步：获取项目信息（不包含billing_type_id，因为projects表没有这个字段）
            ...
            -- 如果仍然没有找到任何链路，使用默认值
            IF chain_id_val IS NULL THEN
                effective_billing_type_id := 1; -- 默认计重
            END IF;
...
-- 为函数添加注释
COMMENT ON FUNCTION public.import_logistics_data(jsonb) IS '修复后的智能导入函数，正确从合作链路获取billing_type_id，不再从projects表获取';

-- 完成提示
SELECT 'import_logistics_data 函数已修复，移除了从 projects 表获取 billing_type_id 的错误逻辑！' as message;

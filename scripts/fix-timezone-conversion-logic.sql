-- 修复时区转换逻辑问题
-- 问题：Excel的2025-08-08转换为UTC后变成2025-08-07，但数据库存储的是2025-08-08
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            -- 修复：直接使用日期字符串进行比较，不进行时区转换
            -- 因为数据库中的日期已经是UTC存储，我们只需要比较日期部分
...
BEGIN
    ...
    final_records AS (
        SELECT 
            ...
            pd.loading_location,
            pd.unloading_location,
            pd.loading_date,        -- 已经是timestamptz
            pd.unloading_date,      -- 已经是timestamptz
            pd.loading_weight,
            pd.unloading_weight,
            pd.current_cost,
            pd.extra_cost,

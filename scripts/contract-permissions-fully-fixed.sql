-- 完全修复的合同权限管理脚本
-- 文件: scripts/contract-permissions-fully-fixed.sql
-- 使用动态SQL解决is_active字段问题
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'contract_permissions' AND table_schema = 'public'
    ) INTO contract_permissions_exists;
    
    -- 检查contract_permissions表的is_active字段
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'contract_permissions' AND column_name = 'is_active' AND table_schema = 'public'
    ...
    RAISE NOTICE 'contract_permissions.is_active字段存在: %', is_active_exists;
    ...
    -- 创建contract_permissions表（如果不存在）
    IF NOT contract_permissions_exists THEN
        ...
        RAISE NOTICE '已创建contract_permissions表';
    END IF;
    
    -- 如果contract_permissions表存在但没有is_active字段，添加它
    IF contract_permissions_exists AND NOT is_active_exists THEN
        ALTER TABLE public.contract_permissions ADD COLUMN is_active BOOLEAN DEFAULT true;
        RAISE NOTICE '已为contract_permissions表添加is_active字段';
    END IF;
...
-- contract_permissions表的RLS策略
DROP POLICY IF EXISTS "Admins can manage all contract permissions" ON public.contract_permissions;
...
DROP POLICY IF EXISTS "Users can view their own permissions" ON public.contract_permissions;
...
-- 12. 创建权限统计函数（简化版本，避免is_active字段问题）
...
BEGIN
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM contract_permissions 
         WHERE is_active = true AND (expires_at IS NULL OR expires_at > NOW())) as active_permissions,

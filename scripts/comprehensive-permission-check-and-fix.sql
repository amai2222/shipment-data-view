...
WHERE user_id IN (
    SELECT u.id 
    FROM public.users u
    WHERE u.role IN ('admin', 'manager', 'operator', 'viewer', 'partner')
)
AND project_id IS NULL
...
-- 7. 创建角色变更触发器
DROP TRIGGER IF EXISTS trigger_role_change ON public.users;
...
-- 12. 创建权限检查视图
CREATE OR REPLACE VIEW user_permission_status AS
SELECT 
    u.id,
    u.email,
    u.role,

-- 创建 import_logistics_data 函数，确保司机和地点正确关联项目  
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            WHERE name = (v_record_data->>'project_name');
            ...
            IF (v_record_data->>'chain_name') IS NOT NULL AND (v_record_data->>'chain_name') != '' THEN
                ...
                WHERE project_id = v_project_id AND chain_name = (v_record_data->>'chain_name');
            END IF;
            ...
            -- 生成运单编号
            SELECT COALESCE(MAX(substring(auto_number from 12)::integer), 0) + 1
            INTO v_daily_seq
            FROM public.logistics_records 
            WHERE loading_date::date = (v_record_data->>'loading_date')::date
            ...
            INSERT INTO public.logistics_records (
                auto_number, project_id, project_name, chain_id,
                ...
                COALESCE((v_record_data->>'current_cost')::numeric, 0),
                COALESCE((v_record_data->>'extra_cost')::numeric, 0),
                COALESCE((v_record_data->>'current_cost')::numeric, 0) + COALESCE((v_record_data->>'extra_cost')::numeric, 0),
                (v_record_data->>'license_plate'),
                (v_record_data->>'driver_phone'),
    ...
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            ...
            'failures', jsonb_build_array(jsonb_build_object(
                'row_index', 0,
                'data', '{}',
                'error', 'Import operation failed: ' || SQLERRM
            ))
        );
END;
$function$;
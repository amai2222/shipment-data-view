...
-- 为性能优化添加必要的索引
CREATE INDEX IF NOT EXISTS idx_logistics_records_performance 
ON logistics_records (project_id, loading_date, created_at);

CREATE INDEX IF NOT EXISTS idx_logistics_partner_costs_performance 
ON logistics_partner_costs (logistics_record_id, partner_id, level);

-- 为partner_chains表添加索引（如果不存在）
CREATE INDEX IF NOT EXISTS idx_partner_chains_project_billing 
ON partner_chains (project_id, billing_type_id);
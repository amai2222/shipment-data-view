-- ============================================================================
-- 修复 get_invoice_requests_filtered_1114 函数类型转换错误
-- ============================================================================
-- 创建日期：2025-11-14
-- 问题：operator is not unique: unknown - unknown
-- 原因：UUID类型参数在format函数中需要显式类型转换
-- 修复：为UUID类型参数添加显式类型转换
-- ============================================================================

BEGIN;

-- 修复get_invoice_requests_filtered_1114函数中的类型转换问题
...
开票申请单筛选查询函数（已添加收款相关字段，支持p_invoicing_partner_id和分页参数，已修复UUID类型转换问题）
- 新增字段：
...
- 修复：
  1. UUID类型参数显式类型转换，解决 operator is not unique 错误
';
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '修复内容：';
    RAISE NOTICE '  - p_invoicing_partner_id: 添加 ::uuid 类型转换';
    RAISE NOTICE '  - p_project_id: 添加 ::uuid 类型转换';
    RAISE NOTICE '  - 解决了 operator is not unique 错误';
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
END $$;

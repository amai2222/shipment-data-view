-- 修复项目管理中合作链路计费模式不生效的问题
-- 1. 修复 get_projects_with_details_fixed 函数，添加 billing_type_id 字段
-- 2. 修复 save_project_with_chains_fixed 函数，处理 billing_type_id 字段

-- 1. 修复项目详情查询函数，添加 billing_type_id 字段
    ...
    SELECT c.project_id,
      jsonb_agg(
        jsonb_build_object(
          ...
          'billing_type_id', c.billing_type_id,  -- ✅ 添加 billing_type_id 字段
...
-- 2. 修复项目保存函数，处理 billing_type_id 字段
...
-- 3. 为现有的 partner_chains 记录设置默认的 billing_type_id（如果为空）
UPDATE public.partner_chains 
SET billing_type_id = 1 
WHERE billing_type_id IS NULL;

-- 4. 添加注释说明
COMMENT ON FUNCTION public.get_projects_with_details_fixed() IS '修复版本：项目详情查询函数，包含 billing_type_id 字段';
COMMENT ON FUNCTION public.save_project_with_chains_fixed(uuid, jsonb, jsonb) IS '修复版本：项目保存函数，支持 billing_type_id 字段的保存';
...
-- 创建货主看板函数 _1120 版本
-- 创建日期：2025-11-20
-- 功能：基于 _1115 版本创建 _1120 版本的货主看板函数
-- ============================================================================

-- ============================================================================
-- 1. get_shipper_dashboard_stats_1120
-- ============================================================================
-- 获取货主看板总体统计数据（包含本级和下级货主）
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_shipper_dashboard_stats_1120(
    p_shipper_id UUID,
...
COMMENT ON FUNCTION public.get_shipper_dashboard_stats_1120 IS '获取货主看板总体统计数据（包含本级和下级货主）- 使用 parent_partner_id 递归查询 - 1120版本';

-- ============================================================================
-- 2. get_subordinate_shippers_stats_1120
-- ============================================================================
-- 获取下级货主列表及统计数据（包含本级和下级）
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_subordinate_shippers_stats_1120(
    p_shipper_id UUID,
    p_start_date DATE DEFAULT NULL,
    p_end_date DATE DEFAULT NULL
)
...
COMMENT ON FUNCTION public.get_subordinate_shippers_stats_1120 IS '获取下级货主列表及统计数据（包含本级和下级）- 使用 parent_partner_id 递归查询 - 1120版本';
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ 货主看板函数已创建（_1120版本）';
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
    RAISE NOTICE '已创建的函数：';
    RAISE NOTICE '  • get_shipper_dashboard_stats_1120';
    RAISE NOTICE '  • get_subordinate_shippers_stats_1120';
    RAISE NOTICE '';
    RAISE NOTICE '关键特性：';
    RAISE NOTICE '  - 使用 parent_partner_id 递归查询所有下级';
    RAISE NOTICE '  - 不依赖 hierarchy_path 字段';
    RAISE NOTICE '  - 支持包含本级和下级的数据统计';
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
END $$;

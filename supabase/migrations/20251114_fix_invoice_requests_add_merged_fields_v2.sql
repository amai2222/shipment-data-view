...
-- 修复：添加 is_merged_request 和 merged_count 到返回字段，修复类型转换问题
-- ============================================================================

BEGIN;

-- 先删除旧版本
DROP FUNCTION IF EXISTS public.get_invoice_requests_filtered_1114(
    TEXT, TEXT, TEXT, DATE, TEXT, UUID, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER, INTEGER, INTEGER
);
...
BEGIN
    ...
    IF p_limit IS NOT NULL AND p_offset IS NOT NULL THEN
        v_limit := p_limit::INTEGER;
        v_offset := p_offset::INTEGER;
    ELSE
        -- 明确类型转换，避免 unknown - unknown 错误
        v_limit := COALESCE(p_page_size, 20)::INTEGER;
        v_offset := ((COALESCE(p_page_number, 1)::INTEGER - 1) * COALESCE(p_page_size, 20)::INTEGER)::INTEGER;
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'page_number', COALESCE(p_page_number, 1),
        'page_size', COALESCE(p_page_size, 20),
        'total_pages', CEIL(v_total_count::NUMERIC / COALESCE(p_page_size, 20)::NUMERIC)
    );
END;
$$;
...
- 修复：
  1. 明确类型转换，解决 operator is not unique 错误
  2. 添加 COALESCE 处理 NULL 值
';
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ get_invoice_requests_filtered_1114 函数已修复（v2）';
    RAISE NOTICE '========================================';
    RAISE NOTICE '修复内容：';
    RAISE NOTICE '  - 添加 is_merged_request 字段';
    RAISE NOTICE '  - 添加 merged_count 字段';
    RAISE NOTICE '  - 修复类型转换问题（operator is not unique）';
    RAISE NOTICE '  - 添加 COALESCE 处理 NULL 值';
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
END $$;

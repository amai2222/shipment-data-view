-- 修复 get_payment_requests_filtered 函数的返回类型
-- 问题：logistics_record_ids 字段类型不匹配
...
RETURNS TABLE (
  ...
  status text,
  notes text,
  logistics_record_ids uuid[],  -- 修复：使用 uuid[] 而不是 text[]
    ...
    IF p_waybill_number IS NOT NULL AND p_waybill_number != '' OR
       ...
       p_project_id IS NOT NULL AND p_project_id != '' THEN
        ...
        WHERE (p_waybill_number IS NULL OR p_waybill_number = '' OR lr.auto_number ILIKE '%' || p_waybill_number || '%')
          ...
          AND (p_loading_date IS NULL OR lr.loading_date::text = p_loading_date)
...
-- 添加函数注释
COMMENT ON FUNCTION get_payment_requests_filtered IS '获取筛选后的付款申请单列表，包含申请金额字段，修复了返回类型';

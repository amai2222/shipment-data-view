...
-- 为运费对账函数添加高级筛选支持
-- ============================================================================
-- 功能：扩展 get_finance_reconciliation_by_partner 函数，支持高级筛选
-- 高级筛选字段：司机姓名、车牌号、司机电话、运单编号、其他平台名称
-- ============================================================================

-- 注意：如果函数不存在，需要先创建基础函数
-- 这里假设函数已存在，我们只需要添加高级筛选参数

-- 由于无法直接修改函数签名，我们需要创建一个新版本或使用函数重载
-- 这里我们创建一个新函数，支持高级筛选

CREATE OR REPLACE FUNCTION public.get_finance_reconciliation_by_partner_1115(
    -- 常规筛选参数
...
COMMENT ON FUNCTION public.get_finance_reconciliation_by_partner_1115 IS 
'获取运费对账数据，支持高级筛选（司机、车牌号、电话、运单编号、其他平台名称）';

-- 验证
SELECT '✅ get_finance_reconciliation_by_partner_1115 函数已创建：添加了高级筛选支持' AS status;

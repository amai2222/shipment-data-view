...
-- 修复标准导入函数：添加合作方成本重算
-- ============================================================================
-- 问题：batch_import_logistics_records_with_update 函数没有调用成本重算
-- 导致：标准导入后 logistics_partner_costs 表为空
-- 解决：在返回结果前调用 recalculate_and_update_costs_for_records
-- ============================================================================
...
'批量导入运单记录，支持更新模式。已修复：添加了合作方成本自动重算功能。';

-- 验证
SELECT '✅ 函数已修复：添加了成本重算调用' AS status;

-- 1) Helper function to check role-based access for financial data
...
-- 2) Update the RPC to redact sensitive partner fields for non-finance/admin users
                ...
                'partner_costs', (
                    SELECT COALESCE(jsonb_agg(
                        jsonb_build_object(
                            ...
                            -- 以下字段为敏感信息，只有财务或管理员可见
                            'full_name', CASE WHEN v_can_view THEN p.full_name ELSE NULL END,
                            'bank_account', CASE WHEN v_can_view THEN p.bank_account ELSE NULL END,
                            'bank_name', CASE WHEN v_can_view THEN p.bank_name ELSE NULL END,
                            'branch_name', CASE WHEN v_can_view THEN p.branch_name ELSE NULL END
                        ) ORDER BY lpc.level
                    ), '[]'::jsonb)

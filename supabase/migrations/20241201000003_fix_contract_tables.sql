...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'contract_number') THEN
    ALTER TABLE public.contracts ADD COLUMN contract_number TEXT UNIQUE;
  END IF;
  
  -- 添加状态字段
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'status') THEN
    ALTER TABLE public.contracts ADD COLUMN status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'expired', 'terminated', 'archived'));
  END IF;
  
  -- 添加优先级字段
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'priority') THEN
    ALTER TABLE public.contracts ADD COLUMN priority TEXT NOT NULL DEFAULT 'normal' CHECK (priority IN ('low', 'normal', 'high', 'urgent'));
  END IF;
  
  -- 添加负责人字段
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'responsible_person') THEN
    ALTER TABLE public.contracts ADD COLUMN responsible_person TEXT;
  END IF;
  
  -- 添加部门字段
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'department') THEN
    ALTER TABLE public.contracts ADD COLUMN department TEXT;
  END IF;
  
  -- 添加保密字段
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'is_confidential') THEN
    ALTER TABLE public.contracts ADD COLUMN is_confidential BOOLEAN NOT NULL DEFAULT false;
  END IF;
  
  -- 添加访问统计字段
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'last_accessed_at') THEN
    ALTER TABLE public.contracts ADD COLUMN last_accessed_at TIMESTAMP WITH TIME ZONE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'access_count') THEN
    ALTER TABLE public.contracts ADD COLUMN access_count INTEGER NOT NULL DEFAULT 0;
  END IF;
END $$;
